...
def generate_sidebar_elements2(state, icon, report, record_widgets):
    ...
    return state
def generate_sidebar_elements3(state, icon, report, record_widgets):
    # Sidebar -- Image/Title
    st.sidebar.image(
        icon, use_column_width=True, caption="View LPI-KCGCN " + report["omic_learn_version"]
    )
...
def main_text_and_data_upload(state, APP_TITLE):
    ...
    **Note:** It is possible to get artificially high or low performance because of technical and biological artifacts in the data.
    ...
    with st.expander("Upload or select sample dataset (*Required)", expanded=True):
        ...
        if file_buffer is not None:
            state["file_name"] = file_buffer.name.split('.')[0]
...
def read_file_content(file_buffer, encoding):
    try:
        file_content = file_buffer.read().decode(encoding)
    except UnicodeDecodeError:
        st.error(f"Failed to decode the file content with encoding {encoding}. Trying fallback encoding.")
        try:
            file_content = file_buffer.read().decode('utf-8', errors='ignore')
        except UnicodeDecodeError:
            st.error("Failed to decode the file content with fallback encoding.")
            return None
    return file_content

def main_text_and_data_uploadkernel(state, APP_TITLE):
    st.title(APP_TITLE)
    st.info(
        """
    **Note:** It is possible to get artificially high or low performance because of technical and biological artifacts in the data.
    ...
    with st.expander("Upload or select sample dataset (*Required)", expanded=True):
        ...
        file_buffer = st.file_uploader(
            "Upload your dataset below", type=["csv", "xlsx", "xls", "tsv", "bed" , "pdb", "gb", "genbank", "fasta", "fa", "txt"]
        )
        st.markdown(
            """**Note:** By uploading a file, you agree to our
                    [Apache License](https://github.com/OmicEra/OmicLearn/blob/master/LICENSE).
        ...
        if file_buffer is not None:
            ...
            # 文件类型
            file_extension = file_buffer.name.split('.')[-1].lower()
            state["file_extension"] = file_extension
            # 文件编码
            encoding = detect_encoding(file_buffer)
            sequences_type = None
            # 读取文件文本
            file_content = file_buffer.read().decode(encoding) if file_extension in ["txt", "fasta", "fa", "bed", "gb", "genbank", "pdb"] else None
            if file_content:
                try:
                    sequences, metadata = process_sequence_file(file_content)
                    st.success("Valid sequence file.")
                except ValueError as e:
                    st.error(str(e))
                    sequences, metadata = None, None
            elif file_extension in ["csv", "tsv", "xlsx", "xls"]:
                file_buffer.seek(0)
                sequences, metadata, sequences_type = read_tabular_file(file_buffer)

                if sequences:
                    st.success("Valid tabular file.")
                else:
                    st.error("Invalid tabular file format.")
                    sequences, metadata = None, None
            else:
                st.error("Invalid sequence file format.")
                sequences, metadata = None, None
            ...
            st.write(metadata)
            # 根据内容进一步判断序列类型
            if state["sequences"] and sequences_type:
                state["sequences_type"] = sequences_type
            elif state["sequences"] and sequences_type is None:
                def clean_sequence(seq):
                    return re.sub(r'[^ACDEFGHIKLMNPQRSTVWY]', '', seq, flags=re.I)
                cleaned_sequences = [clean_sequence(seq) for seq in state["sequences"]]

                if all(re.match(r'^[AUCG]+$', seq, re.I) for seq in state["sequences"]):
                    state["sequences_type"] = "lncRNA"
                elif all(re.match(r'^[ACDEFGHIKLMNPQRSTVWY]+$', seq, re.I) for seq in cleaned_sequences):
                    state["sequences_type"] = "protein"
                else:
                    st.error("Unable to determine sequence type from content.")
